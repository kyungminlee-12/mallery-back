...
public class MemberRepository {
    ...
    public int checkUserId(String userId) {
        try {
            ...
            String checkUserIdQuery = "select exists(select * from graduation.member where user_id = ?)";
    ...
    public int checkPassword(String password) {
        try {
            // 쿼리문의 결과(존재하지 않음(False,0),존재함(True, 1))를 int형(0,1)으로 반환됩니다.
            String checkPasswordQuery = "select exists(select * from graduation.member where password = ?)"; // User Table에 해당 email 값을 갖는 유저 정보가 존재하는가?
    ...
    public Long findMemberIdByUserId(String userId) {
        ...
        String getUsersByUserIdQuery = "select member_id from graduation.member where user_id = ? ";
    ...
    public String findUsernameByUserId(String memberId) {
        String getUsernameQuery = "select username from graduation.member where user_id = ?";
